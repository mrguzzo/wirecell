...
def create_dataframe(file, family):
    ...
    if(family=='NUE'): df.loc[:,'original_file'] = [0]*df.shape[0]
    elif(family=='MC'): df.loc[:,'original_file'] = [1]*df.shape[0]
...
print('   sum of the weights = %s' % sum(df_overlay.weight))
...
print('\n\033[1m   Signal = %s entries\033[0m'%(len(df_nuebar)))
print('\n')
print('      from intrinsic nue = %s (%s)' % (len(df_nuebar[df_nuebar.original_file==0]), len(df_nuebar[df_nuebar.original_file==0])/len(df_nuebar)))
print('      from overlay       = %s (%s)' % (len(df_nuebar[df_nuebar.original_file==1]), len(df_nuebar[df_nuebar.original_file==1])/len(df_nuebar)))
print('\n')
...
print('\n\033[1m   Background = %s entries\033[0m'%(len(df_notnuebar)))
print('\n')
print('      from intrinsic nue = %s (%s)' % (len(df_notnuebar[df_notnuebar.original_file==0]), len(df_notnuebar[df_notnuebar.original_file==0])/len(df_notnuebar)))
print('      from overlay       = %s (%s)' % (len(df_notnuebar[df_notnuebar.original_file==1]), len(df_notnuebar[df_notnuebar.original_file==1])/len(df_notnuebar)))
print('\n')
